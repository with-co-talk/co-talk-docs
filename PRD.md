...
- 알림 설정
- 차단 기능
- 프로필 사진 업로드 (초기에는 텍스트 아바타)
- 타이핑 인디케이터
- 읽음 시간 표시

### 6.2 기술적 제약
- 초기에는 웹앱 (PWA)로 시작, 모바일 앱 (Flutter)은 확장 단계에서 고려
- 초기에는 단일 서버 구성
- 데이터베이스 백업은 일일 1회

### 6.3 플랫폼 전략
- **Phase 1 (MVP)**: 웹앱 (React + PWA) - 모든 플랫폼 접근 가능
- **Phase 2 (확장)**: 모바일 앱 (Flutter) - iOS, Android 동시 지원, 푸시 알림, 백그라운드 실행 지원
- **Phase 3 (선택)**: Flutter Web/Desktop - 웹 및 데스크톱 지원

**이유:**
- 채팅앱은 푸시 알림과 백그라운드 실행이 중요함
...
- 모든 날짜/시간은 서버 시간 기준
- 메시지 ID는 UUID 사용 권장
- 채팅방 ID는 두 사용자 ID 조합 또는 별도 UUID
- 초기 버전은 데스크톱 웹 브라우저 우선 지원
